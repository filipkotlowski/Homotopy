...
<template>
    <div 
        ...
        <vue-feather :type="$props.data?.icon" class="ml-2 text-white" :class="currentRoute == props.data?.path ? 'text-gray-700' : 'text-gray-500'" />
        <span class="ml-2 w-[65%] text-white" :class="currentRoute == props.data?.path ? 'text-gray-700' : 'text-gray-500' ">
    ...
    <ol v-if="isTabOpen" class="w-full animate-fade-down">
        <li v-for="subTab in props.data?.children"
            ...
            :class="currentRoute == `admin/${subTab.path}` ? 'bg-dark-primary' : ''" 
            @click="handleSubTabClick(subTab.path, props.data?.isAdmin)"
            >
            <vue-feather type="circle" size="10" class="ml-2" 
                :class="currentRoute == `admin/${subTab.path}` ? 'text-primary' : ''" />
            <span class="ml-2 w-[75%] py-1" 
                :class="currentRoute == `admin/${subTab.path}` ? 'text-primary font-bold' : ''">
